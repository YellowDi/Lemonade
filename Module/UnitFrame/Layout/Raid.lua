...
local auraFilters = {
        ...
        [1088] = {
            ...
            [211258] = 91, -- Void Ejection (non-dispellable) (DMG + DoT)
        ...
        if aura and aura.show then
            ...
            indicator:SetScript("OnUpdate", function(self, elapsed, ...)
                ...
                if self.timer > 0.10 then
                    ...
                    if indicator.timeLeft > 0 and (indicator.timeLeft - GetTime() < indicator.duration * 0.3) then
...
local function SetLeaderIndicator(self, ...)
    ...
    self.Leader = indicator
end

local function SetLFDRoleIndicator(self, ...)
    local anchor = CreateFrame("Frame", nil, self)
    anchor:SetAllPoints()
    anchor:SetFrameLevel(self.Health:GetFrameLevel() + 1)
...
local function SetMasterLooterIndicator(self, ...)
    ...
    local indicator = anchor:CreateTexture(nil, "ARTWORK")
    indicator:SetSize(12, 12)
    indicator:SetPoint("CENTER", anchor, "BOTTOM", 0, 0)
    
    self.MasterLooter = indicator
end
